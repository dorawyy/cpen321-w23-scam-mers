...
app.post('/player/:playerId/run', async (req, res) => {
  try {
    ...
    // console.log("PLAYER RUN: " + JSON.stringify(playerRun))
    // console.log("PLAYER RUN: " + playerRun)
...
async function updateLobbyMaps(playerId, addedLand) {
  ...
  if (player) {
    ...
    for (let i in playerLobbies) {
      ...
      // console.log("HERE" + JSON.stringify(lobby))
...
export { app, client, playersCollection, lobbiesCollection };